...
"""
Train a custom model on a custom video dataset.

Tutorial (similar to single-image detection): https://github.com/ultralytics/yolov5/wiki/Train-Custom-Data

Usage:
    $ python path/to/train_video.py --data coco128.yaml --weights yolov5s.pt --img 640  # from pretrained (RECOMMENDED)
    $ python path/to/train_video.py --data coco128.yaml --weights '' --cfg yolov5s.yaml --img 640  # from scratch
"""
...
from utils.video_dataloaders import create_seq_dataloader
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = _m(cfg=cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), nt=nt).to(device)  # create
        ...
        model = _m(cfg=cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), nt=nt).to(device)  # create
